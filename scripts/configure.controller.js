angular.module("teachersAlly").controller("configureController", [
  "$scope",
  "$http",
  "$stateParams",
  "$state",
  ...
  function($scope, $http, $stateParams, $state, $timeout) {
    ...
    $http
      .get(
        "./database/" + $stateParams.user + "/" + $stateParams.class + ".json"
      )
      .then(function(res) {
    ...
    $scope.addColumnsOrSubjects = function(e) {
      ...
      $timeout(function() {
        $(_this)
          .parent()
          .find(".config-card-list li")
          .eq($scope.settings[type].length - 1)
          .find("p")
          .focus();
      }, 1);
    };
    ...
    $scope.checkboxStateChange = function(item) {
      ...
      if ($scope[item]) {
        if (item == "Subject_Grading") {
          ...
          // Simply pushes to settings.calculations if item = ranks || overall_grades ||  student_totals
        ...
        if (item == "Subject_Grading") {
          ...
          // Simply removed from settings.calculations if item = ranks || overall_grades ||  student_totals
          $scope.settings.calculations.splice(
            $scope.settings.calculations.indexOf(item)
          );
        }
        $scope.updateSettings($scope.settings, "updateCalculations", item, "");
      }
    ...
    function updateSubject_Grading(oldVal, newVal) {
      if (oldVal == "" && newVal != "") {
        //means subject added thus add its grade
        $scope.settings.sub_gr.gradables.push(newVal + " Grades");
        updateReorderList_SubjectGrades("add", oldVal, newVal);
      } else if (oldVal != "" && newVal == "") {
        //means subject deleted thus delete its grade
        $scope.settings.sub_gr.gradables.splice(
          $scope.settings.sub_gr.gradables.indexOf(oldVal + " Grades"),
          1
        );
        updateReorderList_SubjectGrades("delete", oldVal, newVal);
      } else if (oldVal != "" && newVal != "") {
        //means subject edited thus edit it's grade
        $scope.settings.sub_gr.gradables[
          $scope.settings.sub_gr.gradables.indexOf(oldVal + " Grades")
        ] =
          newVal + " Grades";
    ...
    function updateReorderList_SubjectGrades(action, oldVal, newVal) {
      switch (action) {
        case "add":
          $scope.settings.reorderList.splice(
            $scope.settings.reorderList.indexOf(newVal) + 1,
            0,
            newVal + " Grades"
          );
          break;
        case "delete":
          $scope.settings.reorderList.splice(
            $scope.settings.reorderList.indexOf(oldVal + " Grades"),
            1
          );
          break;
        case "edit":
    ...
    function buildReorderList() {
      // for reorder list
      $scope.settings.reorderList = [];
      $scope.settings.reorderList = $scope.settings.reorderList.concat(
    ...
    function updateReorderList(oldVal, newVal, action) {
      function update(oldVal, newVal) {
        if (oldVal != "" && newVal != "") {
          ...
          $timeout(function() {
            $(".table-preview-container").mCustomScrollbar(
              "scrollTo",
              "last",
              {
          ...
          $timeout(function() {
            $(".table-preview-container:eq(2) table")
              .parent()
              .css(
                "width",
                $(".table-preview-container:eq(2) table").width() + "px"
              );
          }, 1);
        } else {
